...
void HashTable::Insert(Bid bid) {
    ...
    // search if node exists. If doesnt exist and set to default construtor, overwrite
    if nodes.at(tempKey).key == UNIT_MAX{
    ...
    // Navigate to the bottom of the linkedList
    else {

        // Create a cursor node to iterate and point at the hash location
        Node* currNode = &nodes.at(tempKey);

        // Create node pointer
        Node* newNode = new Node(bid, tempKey);

        // Iterate through the linkedList to find the last position
...
void HashTable::PrintAll() {
    ...
    // Declare test variable to zero
    int j = 0;

    // iterate through bids, begin to end node
    for (unsigned int i = 0; i < tableSize; i++) {

        Node* currNode = &nodes.at(i);

        if (currNode->key != UNIT_MAX) {
...
void HashTable::Remove(string bidId) {
    ...
    // Logic setting key equal to hash atoi. cursor node to head node
    Node* cursor = &nodes.at(hash(stoi(bidId)));
    
    Node* tempNode = NULL;

    /*
     * Use scan and shift logic if there is a chain of bids.If no chain, check head node.
     * If bucket head is not null, there is a chain
     */
    if (cursor->next != nullptr) {

        // special case when cursor->bid.bidId matches the head of chain
        if (cursor->bid.bidId == bidId) {

            // Point tempNode to the next node
        ...
        // scan chain for a match
            ...
            // Hold next cursor value in tempNode
    ...
    // Match head node with no chain
    else {
        
        if (cursor->bid.bidId == bidId) {

            // Set bucket back to original
            Node newNode = Node();
