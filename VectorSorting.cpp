...
int partition(vector<Bid>& bids, int begin, int end) {
    ...
    int middlePoint = (begin + (end - begin) / 2); // Identify middlePoint for partition
    int pivot = middlePoint; // Pivot variable set as element in Partition function
    bool finished = false; // True/false logic to continue or end Partition

    // while not done 
    while (!finished) {

        // Increment the low point as long as it is below pivot
        while (bids[low].title.compare(bids[pivot].title) < 0) {
            ++low;
        }

        // Increment the high point as long as it is above the pivot
        ...
        // If low bid is not greater than or equal to the high point, swap the bids
        else {
...
void selectionSort(vector<Bid>& bids) {
    ...
    for (int pos = 0; pos < max; ++pos) {
        ...
        // Loop thorugh positions to designate which elements are swapped
...
int main(int argc, char* argv[]) {
    ...
    // Output menu selection for user input
    int choice = 0;
    while (choice != 9) {
        cout << "Menu:" << endl;
